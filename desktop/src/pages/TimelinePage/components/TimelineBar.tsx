...
export const TimelineBar: React.FC<TimelineBarProps> = ({ timelineData, onJumpToSection }) => {
  ...
  // Build timeline segments using useMemo to prevent recreating on every render
  ...
  // Extract unique years for year markers
  const years = useMemo(() => {
    const uniqueYears = new Set(segments.map((s) => s.year));
    return sortYears(Array.from(uniqueYears));
  }, [segments]);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      for (const segment of segments) {
        ...
        if (
          detectionPoint >= accumulatedHeight &&
          detectionPoint < accumulatedHeight + monthHeight
        ) {
      ...
      if (!foundActive && segments.length > 0) {
        ...
        if (firstSegment) {
          setActiveSegment(firstSegment.id);
        }
      }
    };
        ...
        {segments.map((segment) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`absolute left-1/2 -translate-x-1/2 rounded-full border-2 border-background transition-all duration-300 ${
                  isActive
