...
export const ListItem: React.FC<ListItemProps> = ({ item, onClick }) => {
  ...
  const renderFolderIcon = () => {
    ...
    if (item.useColoredCard) {
      const config =
        item.hierarchyLevel === 'category'
          ? getSubthemeCategoryConfig(item.displayName)
          : getSubclassificationConfig(item.displayName);

      return <div className="p-2 bg-background/80 backdrop-blur rounded-md">{config.icon}</div>;
    }
  ...
  return (
    ...
    >
          ...
          {item.type === 'folder' && (
            <>
              <p className="text-xs text-muted-foreground mt-1">
                {item.itemCount} {item.itemCount === 1 ? 'document' : 'documents'} •{' '}
                {formatFileSize(item.size ?? 0)}
              </p>
              {item.processingCount && item.processingCount > 0 && (
                <p className="text-xs text-yellow-600">{item.processingCount} still processing</p>
              )}
            </>
          )}
          {item.type === 'file' && (
            <>
              ...
              {item.primaryTheme && (
                <p className="text-xs text-muted-foreground">Theme: {item.primaryTheme}</p>
              )}
            </>
          )}
        </div>
      ...
      <div className="flex items-center space-x-6">
        {item.type === 'file' && (
          <>
            <span className="text-sm text-muted-foreground">{formatFileSize(item.size ?? 0)}</span>
        ...
        {item.type === 'folder' && <ChevronRight className="h-4 w-4 text-muted-foreground" />}
      </div>
    </div>
  );
};
