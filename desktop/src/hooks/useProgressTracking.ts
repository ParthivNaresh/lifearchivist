...
export const useProgressTracking = (
  ...
  // Use a ref to store the function to allow recursive calls
  const createWebSocketConnectionRef =
    useRef<(sessionId: string, fileId: string) => Promise<WebSocket>>();

  // Define the WebSocket connection function
  const createWebSocketConnection = useCallback(
    (sessionId: string, fileId: string): Promise<WebSocket> => {
      return new Promise((resolve, reject) => {
        ...
        ws.onclose = (event) => {
          ...
          const hasActiveConnections = Array.from(sessionsRef.current.values()).some(
            (s) => s.isConnected
          );
          ...
          if (event.code !== 1000 && event.code !== 1001) {
            const reconnectTimeout = setTimeout(() => {
              if (sessionsRef.current.has(sessionId)) {
                ...
                // Recursive call to reconnect using ref
                createWebSocketConnectionRef
                  .current?.(sessionId, fileId)
                  .then((newWs) => {
                    const session = sessionsRef.current.get(sessionId);
                    if (session) {
  ...
  const createSession = useCallback(
    async (fileId: string): Promise<string> => {
      ...
      try {
        console.log(
          `🔗 Creating progress tracking session for file ${fileId} with session ${sessionId}`
        );
